> 整理来自 https://liaoxuefeng.com/books/java/oop/basic/inherit/index.html
...
仔细观察，发现Student类包含了`Person`类已有的字段和方法，只是多出了一个`score`字段和相应的`getScore()`、`setScore()`方法。
...
> 最新的jdk25 中，构造方法的第一行已经可以允许不是super()或this()，但为了兼容性，建议仍然遵守这个规则。
...
究其原因，是因为`Student`是`Person`的一种，它们是is关系，而`Student`并不是`Book`。实际上`Student`和`Book`的关系是`has`关系。
