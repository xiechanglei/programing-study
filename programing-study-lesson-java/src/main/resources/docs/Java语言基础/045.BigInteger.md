> 整理来自 https://liaoxuefeng.com/books/java/oop/core/biginteger/index.html

在Java中，由CPU原生提供的整型最大范围是64位`long`型整数。使用`long`型整数可以直接通过CPU指令进行计算，速度非常快。

如果我们使用的整数范围超过了`long`型怎么办？这个时候，就只能用软件模拟一个大整数，`java.math.BigInteger`类就是用来表示任意大小整数的类。`BigInteger` 内部用一个`int[]` 数组来模拟一个非常大的整数。
...
如果`BigInteger`的值甚至超过了`float`的最大范围`（3.4x1038）`，那么返回的`float`是什么呢？
