> 整理来自 https://liaoxuefeng.com/books/java/quick-start/flow/if/index.html
...
根据if的计算结果（`true`还是`false`），JVM决定是否执行`if`语句块（即花括号{}包含的所有语句）。
...
由于使用缩进格式，很容易把两行语句都看成`if`语句的执行块，但实际上只有第一行语句是if的执行块。在使用`git`这些版本控制系统自动合并时更容易出问题，所以不推荐忽略花括号的写法。
...
执行发现，`n = 100`时，满足条件`n >= 90`，但输出的不是"优秀"，而是"及格了"，原因是if语句从上到下执行时，先判断`n >= 60`成功后，后续`else`不再执行，因此，`if (n >= 90)`没有机会执行了。
