> 整理来自 https://liaoxuefeng.com/books/java/exception/commons-logging/index.html
...
- `FATAL`：致命错误，系统崩溃前的最后记录；
- `ERROR`：错误信息，不影响系统运行，但需要关注和修复；
- `WARNING`：警告信息，提示可能出现的问题；
- `INFO`：一般信息，系统运行状态的正常记录；
- `DEBUG`：调试信息，开发和调试阶段使用；
- `TRACE`：跟踪信息，最详细的信息。
