> 整理来自 https://liaoxuefeng.com/books/java/quick-start/flow/switch/index.html
...
如果`option`的值没有匹配到任何`case`，例如`option = 99`，那么，`switch`语句不会执行任何语句。这时，可以给`switch`语句加一个
`default`，当没有匹配到任何`case`时，执行`default`：
...
`yield` 关键字的提出,主要是为了解决`switch` 的`case` 代码块中无法使用`return`语句的问题。