> 整理来自 http://liaoxuefeng.com/books/java/oop/core/string-builder/index.html
...
`StringBuilder`和`StringBuffer`接口完全相同，现在完全没有必要使用`StringBuffer`。实际在我们常规的编程实践中，一般不会有多个线程同时操作同一个字符串拼接对象的情况，因此，`StringBuffer`并不常用，推荐使用`StringBuilder`。
