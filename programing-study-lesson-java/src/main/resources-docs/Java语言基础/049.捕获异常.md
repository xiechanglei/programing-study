> 整理来自 https://liaoxuefeng.com/books/java/exception/java-exception/index.html
...
因为使用`int`类型的错误码，想要处理就非常麻烦。这种方式常见于底层C函数,rust语言中也有类似的`Result`类型。
...
- `OutOfMemoryError`：内存耗尽，程序崩溃；
- `StackOverflowError`：栈溢出，递归调用过深。
- `NoClassDefFoundError`：找不到类定义，通常是因为类路径错误。
- ......
...
- `FileNotFoundException`：文件未找到，提示用户检查文件路径；
- `IOException`：输入输出错误，提示用户重试操作；
- `SQLException`：数据库操作错误，提示用户稍后重试；
- ......

还有一些异常是程序逻辑编写不对造成的，应该修复程序本身。例如：

- `NullPointerException`：空指针异常，检查代码逻辑；
- `ArrayIndexOutOfBoundsException`：数组下标越界，检查数组访问
...
可见，只要是方法声明的`Checked Exception`，不在调用层捕获，也必须在更高的调用层捕获。所有未捕获的异常，最终也必须在`main()`方法中捕获，不会出现漏写try的情况。这是由编译器保证的。`main()`方法也是最后捕获`Exception`的机会。
