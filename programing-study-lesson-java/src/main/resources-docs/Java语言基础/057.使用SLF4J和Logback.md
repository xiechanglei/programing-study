> 整理来自 https://liaoxuefeng.com/books/java/exception/slf4j-logback/index.html
...
使用`SLF4J`和`Logback`和前面讲到的使用`Commons Logging`加`Log4j`是类似的，先分别下载`SLF4J`和`Logback`，然后把以下`jar`包放到`classpath`下：
...
和`Log4j`类似，我们仍然需要一个`Logback`的配置文件，把`logback.xml`放到`classpath`下，配置如下：
