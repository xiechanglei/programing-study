> 整理来自 https://liaoxuefeng.com/books/java/oop/basic/classpath-jar/index.html
...
我们强烈不推荐在系统环境变量中设置`classpath`，那样会污染整个系统环境。在启动JVM时设置`classpath`才是推荐的做法。实际上就是给java命令传入`-classpath`参数：
