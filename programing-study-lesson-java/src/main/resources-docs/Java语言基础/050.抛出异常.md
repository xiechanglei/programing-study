> 整理来自 https://liaoxuefeng.com/books/java/exception/throw-exception/index.html
...
当然，这种语法也有适用的场景，比如在`try`中打开文件，在`finally`中关闭文件时，如果关闭文件时抛出异常，我们仍然希望保留`try`中抛出的异常。